...
namespace UGridNET
{
    internal static class IntPtrHelpers
    {
        ...
        /// <returns>
        /// A pointer to the allocated unmanaged memory if <paramref name="count"/> is strictly positive, IntPtr.Zero
        /// otherwise .
        /// </returns>
        ...
        /// <param name="count">The number of elements to allocate memory for.</param>
        /// <returns>
        /// A pointer to the allocated and zero-initialized unmanaged memory if <paramref name="count"/> is strictly
        /// positive, IntPtr.Zero otherwise.
        /// </returns>
        /// <remarks> The caller is responsible for freeing the memory using <see cref="Free(ref System.IntPtr)"/>. </remarks>
        public static IntPtr AllocateZeroed<T>(int count) where T : struct
        {
            IntPtr ptr = Allocate<T>(count);
            if (ptr == IntPtr.Zero)
            {
                return ptr;
            }

            int totalBytes = count * Marshal.SizeOf<T>();
