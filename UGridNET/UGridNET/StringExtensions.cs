...
namespace UGridNET
{
    namespace Extensions
    {
        ...
        internal static class StringExtensions
        {
            ...
            /// Thrown when the string length exceeds the allowed length minus one for the null
            /// terminator.
            /// </exception>
            public static byte[] GetRightPaddedNullTerminatedBytes(this string str, int length)
            {
                ...
                string paddedString = str.Trim().PadRight(length - 1);

                // Convert the string to bytes
                byte[] bytesFromPaddedString = Encoding.UTF8.GetBytes(paddedString);
            ...
            public static List<string> Tokenize(this string str, int maxTokenLength)
            {
                ...
                for (var i = 0; i < str.Length; i += maxTokenLength)
                {
                    string token = str.Substring(i, maxTokenLength);
