...
namespace UGridNET.Tests
{
    ...
    public class UGridNETExceptionTests
    {
        ...
        public void UGridNETExceptionWithFormattedString()
        {
            try
            {
                throw new UGridNETException("Test exception {0} {1} {2} {3} {2} {1} {0}", 1, 2, 'c', "str");
            }
            catch (UGridNETException exception)
            {
                Assert.That(exception, Is.Not.Null);
                Assert.That(exception.Message, Is.EqualTo("UGridNET error: Test exception 1 2 c str c 2 1"));
            }
        }
