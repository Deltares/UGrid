...
namespace UGridNET.Tests
{
    public class UGridTests
    {
        ...
        {
            int fileMode = -1;
            int result = UGrid.ug_file_read_mode(ref fileMode);
            Assert.That(result, Is.EqualTo(0));

            int fileID = -1;
            string filePath = TestHelper.GetTestFilePath("ResultFile.nc");
            result = UGrid.ug_file_open(Encoding.Default.GetBytes(filePath), fileMode, ref fileID);
            Assert.That(result, Is.EqualTo(0));

            var dimensionsCount = 0;
            var variableNameStr = "mesh1d_s0                                                                       ";
            byte[] variableName = Encoding.Default.GetBytes(variableNameStr);
